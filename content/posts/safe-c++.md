...
One year ago, the [Safe C++ proposal](https://safecpp.org/draft.html) was made. The goal was to add a safe subset/context into C++ that gives strong guarantees (memory safety, type safety, thread safety) similar to what Rust provides, without breaking existing C++ code. It is an extension or superset of C++. The opt-in was marking parts of the code explicitly so delineate the *safe context*. 

> Code in the safe context exhibits the same strong safety guarantees as code written in Rust.

The rest remains "unsafe" in the usual C++ sense. This means that existing code continues to work, while new or refactored parts can gain safety. For those who write Rust, Safe C++ has many similarities with Rust, sometimes with adjustments to fit C++'s design. In fact, Safe C++ retains more of C++'s features: templates, overloading, inheritance, exceptions ... whereas Rust chooses to not have some of these for simplicity and safety. Also, because C++ already has a huge base of "unsafe code", Safe C++ has to provide mechanisms for mixing safe and unsafe, and for incremental migration. In that sense, all of Safe C++'s safe features are opt-in. Existing code compiles and works as before. Introducing safe context doesn’t break code that doesn’t use it.
...
So I went to read the documents related to Profiles[1][2][3][4]. I try to summarize what I understood: Profiles are meant to defined modes of C++ that impose constraints on how you use the language and library, in order to guarantee certain safety properties. So these are restrictions of existing features, not new language features per se. The idea is that you can enable a profile; code using it agrees to follow the restrictions. If you don’t enable it, things work as before. So it's backwards-compatible. 

> Failure to satisfy a constraint dictated by a profile is a diagnosable rule.
...
I think Safe C++ was more ambitious: introducing new syntax, type qualifiers, safe vs unsafe contexts, etc. Some in the committee felt that was too heavy, and Profiles are seen as a more pragmatic path. The main objection is obvious: one could say that Profiles restrict less than what Safe C++ aimed to provide. 
