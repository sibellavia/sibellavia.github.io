...
[DeepSeek-V3.2-Exp](https://sibellavia.lol/posts/2025/09/30/deepseek-v3.2-exp-claude-sonnet-4.5-and-more/) is defined as an *experimental sparse-attention model*. Its architecture is the same as DeepSeek-V3.1-Terminus, except for the introduction of DeepSeek Sparse Attention (DSA). The core idea is to reduce the number of key-value pairs each query token looks at, instead of attending to all tokens. Sparse attention only computes a subset of entries, making long-context reasoning more feasible. 
